...
mkdir -p "$STARTUPDIR/log"
rm -f $STARTUPDIR/log/*.log
...
$NO_VNC_HOME/utils/launch.sh --vnc localhost:$VNC_PORT --listen $NO_VNC_PORT &> $STARTUPDIR/log/no_vnc_startup.log &
PID_SUB=$!

echo -e "\n------------------ start VNC server ------------------------"
echo "remove old vnc locks to be a reattachable container"
vncserver -kill $DISPLAY &> $STARTUPDIR/log/vnc_startup.log \
    || rm -rfv /tmp/.X*-lock /tmp/.X11-unix &> $STARTUPDIR/log/vnc_startup.log \
...
if [[ $DEBUG == true ]] || [[ $1 =~ -t|--tail-log ]]; then
    ...
    tail -f $STARTUPDIR/log/*.log $HOME/.vnc/*$DISPLAY.log
fi
